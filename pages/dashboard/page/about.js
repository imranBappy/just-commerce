...
const AboutPageSetup = () => {
  ...
  useEffect(() => {
    if (data && data.page) {
      setEditorState(data.page.aboutPage.content);
    }
  }, [data]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await postData(`/api/page?scope=about`, {
        content: editorState,
      });
      response.success
  ...
  return (
    <>
      {error ? (
        ...
        <div>
          <div className="py-3">
            <h1 className="h3 text-center pb-3">{t("about_us")}</h1>
            <TextEditor
...
AboutPageSetup.requireAuthAdmin = true;
AboutPageSetup.dashboard = true;

export default AboutPageSetup;
